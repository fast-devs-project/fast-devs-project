...
async function renderApps() {
  ...
  const availableApps = data.filter(app => app.availability !== 'removed');
  const removedApps = data.filter(app => app.availability === 'removed');
  ...
  function renderAppCard(app, i) {
    ...
    const isRemoved = app.availability === 'removed';
    const storeAction = app.appstore
      ...
      : '';
    const landingAction = app.landing
      ? `<a href="${app.landing}" class="btn-app secondary">${t('apps.cta.landing')} <span aria-hidden="true">→</span></a>`
      : '';
    const removedBadge = isRemoved
      ? `<span class="app-removed-badge" title="${t('apps.status.removed.title')}">${t('apps.status.removed')}</span>`
      : '';
  ...
  function renderGroup(titleKey, apps, variant, delayOffset = 0) {
    if (apps.length === 0) return '';

    return `
      <div class="apps-group apps-group-${variant} reveal reveal-delay-${delayOffset + 1}">
        <div class="apps-group-heading">
          <h3>${t(titleKey)}</h3>
          <span>${apps.length}</span>
        </div>
        <div class="apps-grid">
          ${apps.map((app, i) => renderAppCard(app, i + delayOffset)).join('')}
        </div>
      </div>`;
  }

  grid.innerHTML = [
    renderGroup('apps.group.available', availableApps, 'available', 0),
    renderGroup('apps.group.unavailable', removedApps, 'unavailable', availableApps.length)
  ].join('');
