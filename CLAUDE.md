...
### i18n pattern (consistent across all three sites)
...
- `scripts/cache-bust.sh` rewrites the `?v=` values in the three `index.html` files (default: UTC timestamp; `commit` = git short hash; or a custom value).
...
### App landing page sites (device-monitor / iwindrose)
